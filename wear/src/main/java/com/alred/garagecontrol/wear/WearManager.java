            ...
            public void run() {
                ...
                for (Node node : nodes) {
                    ...
                    if (!messageResult.getStatus().isSuccess()) {
                        ...
                        errIntent.setAction(Constants.MESSAGE_ACTION);
                        errIntent.putExtra(Constants.MESSAGE_EXTRA, "ERROR - Phone not connected.");
                        mCtx.sendBroadcast(new Intent(Constants.MESSAGE_ACTION));
                    }
                }
            }
        }).start();
